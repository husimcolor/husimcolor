...
export function interpretEnergyFlow(score: OhangScore): EnergyFlowResult {
  ...
  if (dom === '화' && isOverloaded) {
    return {
      title: '에너지가 빠르게 타오르는 시기',
      description:
        '지금은 감정이 빠르게 반응하고 자극에 예민해져 있습니다. ' +
        '열정과 추진력이 강하게 살아있지만, 그만큼 몸이 쉽게 지치고 마음이 과열될 수 있습니다. ' +
        '머릿속이 분주하거나 잠들기 어려운 날이 이어진다면, 지금 몸이 쉬고 싶다는 신호입니다.',
      recovery:
        '속도를 조금 낮추고 호흡을 고르는 시간이 필요합니다. ' +
        '차가운 물 한 잔, 짧은 산책, 조용한 음악 한 곡이 몸의 긴장을 부드럽게 풀어줍니다.',
  ...
  if (dom === '수' && isOverloaded) {
    return {
      title: '감정이 깊이 가라앉아 있는 시기',
      description:
        '지금은 감정이 안쪽으로 깊이 가라앉아 있고, 몸도 쉽게 피로해집니다. ' +
        '생각이 많아지거나 수면 리듬이 흐트러질 수 있습니다. ' +
        '혼자 감정을 담아두다가 어느 순간 무거워지는 패턴이 반복된다면, 지금 따뜻한 연결이 필요한 때입니다.',
      recovery:
        '지금은 따뜻한 것들이 필요합니다. ' +
        '따뜻한 차 한 잔, 신뢰하는 사람과의 짧은 대화, 몸을 따뜻하게 감싸는 루틴이 마음을 조금씩 녹여줍니다.',
  ...
  if (dom === '토' && isOverloaded) {
    return {
      title: '무겁고 답답한 감정이 쌓이는 시기',
      description:
        '지금은 안정을 원하지만 동시에 답답함이나 무거움이 느껴집니다. ' +
        '변화보다 익숙한 것을 선택하게 되고, 감정이 잘 움직이지 않고 고여 있는 느낌이 들 수 있습니다. ' +
        '소화가 잘 안 되거나 몸이 무겁게 느껴지는 날이 있다면, 몸과 마음 모두 환기가 필요한 신호입니다.',
      recovery:
        '작은 움직임 하나가 막힌 감정을 다시 열어줍니다. ' +
        '산책, 가벼운 스트레칭, 평소와 다른 장소에서 커피 한 잔처럼 작은 변화가 에너지를 풀어줍니다.',
      balanceKeywords: ['움직임', '작은 변화', '환기'],
    };
  }
  ...
  if (dom === '금' && isOverloaded) {
    return {
      title: '주변을 정리하고 비우고 싶은 시기',
      description:
        '지금은 주변을 정리하고 싶은 마음이 강하게 올라옵니다. ' +
        '감정이나 관계에서 불필요한 것을 덜어내고 싶고, 혼자만의 조용한 공간이 필요하게 느껴집니다. ' +
        '예민함이 높아져 작은 자극에도 쉽게 피로해지는 날이 있다면, 지금은 비우는 것이 채우는 것보다 중요합니다.',
      recovery:
        '공간을 비우고 조용한 시간을 갖는 것이 지금 가장 좋은 회복입니다. ' +
        '방 한 구석 정리하기, 불필요한 연락 잠시 줄이기, 아무것도 하지 않는 30분이 마음의 여백을 만들어줍니다.',
  ...
  if (dom === '목' && isOverloaded) {
    return {
      title: '새로운 것을 향해 뻗어나가는 시기',
      description:
        '지금은 새로운 것을 배우고 확장하려는 욕구가 강하게 살아있습니다. ' +
        '관심사가 여러 방향으로 뻗어나가며 에너지가 분산되기 쉽습니다. ' +
        '무언가를 이루고 싶은 마음이 크지만, 방향이 너무 많아 정작 중요한 것을 놓칠 수 있습니다.',
      recovery:
        '지금은 한 가지에 집중하는 시간이 필요합니다. ' +
        '가장 중요한 것 하나를 정하고, 나머지는 잠시 내려놓는 것이 에너지를 모아줍니다. ' +
        '빠르게 달리는 것보다 방향을 확인하는 것이 더 중요한 시기입니다.',
  ...
  if (weak === '화' && ratio(score, '화') < 0.1) {
    return {
      title: '활력과 표현 에너지가 필요한 시기',
      description:
        '지금은 감정 표현이나 행동력이 조금 낮아져 있습니다. ' +
        '의욕이 잘 살아나지 않거나, 하고 싶은 것이 있어도 실행이 느려지는 때입니다. ' +
        '억지로 움직이기보다 작은 즐거움 하나를 먼저 찾아보는 것이 좋습니다.',
      recovery:
        '작은 행동 하나가 활력을 되살려줍니다. ' +
        '좋아하는 음악 듣기, 가벼운 산책, 오랫동안 미뤄왔던 것을 딱 5분만 시작해보는 것이 도움이 됩니다.',
  ...
  if (weak === '수' && ratio(score, '수') < 0.1) {
    return {
      title: '안으로 쉬는 시간이 필요한 시기',
      description:
        '지금은 바깥으로 향하는 에너지가 강하고, 안으로 쉬는 시간이 부족합니다. ' +
        '생각을 정리하거나 감정을 돌아보는 시간이 줄어들어 있을 수 있습니다. ' +
        '바쁘게 움직이는 것이 오히려 내면의 피로를 쌓고 있는 때일 수 있습니다.',
      recovery:
        '하루 중 조용히 혼자 있는 시간을 의식적으로 만들어보세요. ' +
        '짧은 일기 쓰기, 5분 명상, 이어폰 없이 걷는 산책이 내면의 균형을 되찾아줍니다.',
  ...
  // 기본 균형 상태
  return {
    title: '안정된 에너지가 유지되는 시기',
    description:
      '지금은 감정과 몸의 에너지가 비교적 고르게 유지되고 있습니다. ' +
      '특정 방향으로 치우치지 않고 여러 에너지가 함께 살아있는 상태입니다. ' +
      '지금 이 안정감을 유지하면서 자신에게 필요한 것을 조금씩 채워가는 것이 좋습니다.',
    recovery:
      '지금의 안정을 유지하면서 자신을 조금 더 돌봐주는 시간을 만들어보세요. ' +
      '작은 루틴 하나를 꾸준히 이어가는 것이 이 상태를 더 단단하게 만들어줍니다.',
...
const ARCHETYPE_MAP: Record<LifeArchetypeKey, LifeArchetype> = {
  ...
  healer: {
    ...
    drainPattern: '타인의 감정을 너무 깊이 흡수하다가 자신이 지치는 경향이 나타날 수 있습니다.',
  ...
  leader: {
    ...
    dailyPractice: '오늘 이끄는 역할에서 잠시 내려와 스스로 쉬는 시간을 허락하세요.',
  },
  artist: {
    key: 'artist',
...
function buildContextualRoutinesForFlow(
  ...
  const baseRoutinePool: Record<FlowType, string[]> = {
    '진장': [
      '저녁 스마트폰 사용을 1시간 일시 중단하기',
      '자기 전 따뜻한 물 한 잔 마시며 속도 낮추기',
      '코에서 시작하는 짧은 복식호흡 (4초 들이쉬고 내쉬기)',
      '오늘 하루 중 가장 진장된 순간을 일기에 적어보기',
    ],
    '침잠': [
      '낮 시간에 10분 햇빛 속에 앉아 있기',
  ...
  const archetypeRoutineAdd: Record<LifeArchetypeKey, Record<FlowType, string>> = {
    ...
    healer: {
      ...
      '정체': '감정 일기를 써서 안에 담아둔 감정을 바깥으로 꺼내기',
    ...
    analyst: {
      ...
      '침잠': '생각을 정리하는 노트를 쓰되 결론은 내지 않아도 되는 방식으로 쓰기',
    ...
    artist: {
      ...
      '침잠': '좋아하는 음악을 틀며 감정을 지금 이대로 느끼기',
      '정체': '작은 창작물 (노트, 스케치, 사진)으로 막힌 표현을 풀어내기',
    ...
    expert: {
      ...
      '침잠': '지식보다 지금 자신의 머리와 몸을 쉬게 하는 것을 우선하기',
      '정체': '깊이 파고들던 주제를 잠시 닫아두고 다른 것에 시선 돌리기',
  ...
  const flowCoaching: Record<FlowType, string> = {
    '진장': '지금 속도를 조금 낮춰도 괜찮다는 것을 스스로에게 허락하세요.',
