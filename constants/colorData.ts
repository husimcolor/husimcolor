...
export const COLOR_DATA: ColorData[] = [
  ...
  },
  {
    ...
    recoveryMessages: [
      '쌓여있던 감정들을 조용히 흘려보내는 시간이 필요합니다.',
      '정화는 억지로 하는 것이 아닙니다.\n천천히 감정이 자연스럽게 흐르도록 두세요.',
      '내면의 무거움을 내려놓을 때,\n비로소 맑은 에너지가 채워집니다.',
    ],
  },
  {
  ...
  },
  {
    ...
    recoveryMessages: [
      '자신을 더 깊이 들여다보는 시간이 필요합니다.\n당신 안에는 생각보다 많은 것이 있습니다.',
      '자신을 과소평가하지 마세요.\n지금의 당신은 충분히 소중하고 가치 있습니다.',
      '자기 인식이 깊어질수록\n삶이 더 풍요로워집니다.',
    ],
  },
  {
  ...
  },
  {
    ...
    recoveryMessages: [
      '비워낸 자리에\n따뜻한 생기를 다시 채워볼 시간입니다.',
      '새로운 시작을 위해\n작은 기쁨과 활력을 먼저 찾아보세요.',
      '공허함은 새로운 것을 채울 준비가 된 상태입니다.\n오늘은 무엇이 당신에게 생기를 주는지 생각해 보세요.',
    ],
  },
  {
    id: 'silver',
...
export function generateInterpretation(
  ...
  // 코칭 메시지: 3번 카드(회복 방향)의 recoveryMessages 중심으로 생성
  // 1번 카드(현재 상태)와 3번 카드(회복 방향)의 조합으로 맞춤 메시지 우선 적용
...
/**
 * 코칭 메시지 생성 - 3번 카드(회복 방향) 중심
 * 현재 상태(1번)를 인식하고, 회복 방향(3번)으로 연결하는 메시지 생성
 */
function generateCoachingMessage(card1: ColorData, card2: ColorData, card3: ColorData): string {
  // 주요 조합별 맞춤 코칭 메시지 (현재 상태 → 회복 방향 연결)
  const coachingCombos: Record<string, string> = {
    // 아이보리/크림 + 스카이블루 → 오렌지 (조용함에서 따뜻한 연결로)
    'cream_skyblue_orange': '혼자 머물러 있던 마음에\n다시 따뜻한 연결이 필요합니다.',
    // 레드 + 블루 → 화이트 (긴장에서 정화로)
    'red_blue_white': '강하게 버텨온 마음을\n이제는 조용히 비워도 됩니다.',
    // 레드 + 블루 → 그린 (긴장에서 회복으로)
    'red_blue_green': '달려온 에너지를 잠시 내려놓고,\n자연스러운 회복의 흐름에 몸을 맡겨보세요.',
    // 블루 + 화이트 → 그린 (책임감에서 균형으로)
    'blue_white_green': '책임감으로 가득 찬 마음에\n균형과 회복의 에너지가 필요합니다.',
    // 블루 + 그린 → 핑크 (침묵에서 따뜻함으로)
    'blue_green_pink': '혼자 감당해온 마음에\n따뜻한 배려와 돌봄이 필요합니다.',
    // 그린 + 핑크 → 세이지 (회복과 배려에서 자기표현으로)
  ...
  // 2카드 조합 체크
  const key2 = `${card1.id}_${card3.id}`;
